...
import { platformInfo, hasNative, storageInfo, netInfo } from "./transport.js";
  ...
  if (app && app.world) {
    ...
    out.push(["terreno", !w.terrainMesh
      ...
      : (w.terrainKnown
        ...
        : "PLACEHOLDER PLANO — no ha llegado ningún parche de terreno del sim (el agua se mantiene oculta hasta que llegue)")]);
  }
...
/** Downloads every bundled avatar asset and un-gzips it (they all ship in the APK). */
export async function assetCheck() {
  const out = [];
  const base = new URL("../data/avatar/", import.meta.url);
  for (const name of AVATAR_ASSETS) {
    const url = new URL(name, base).href;
    try {
      const res = await fetch(url);
      if (!res.ok) { out.push([name, false, `HTTP ${res.status}`]); continue; }
      const buf = await res.arrayBuffer();
      try {
        const plain = await gunzip(buf);
        out.push([name, true, `${(buf.byteLength / 1024).toFixed(0)} KB → ${(plain.length / 1024).toFixed(0)} KB`]);
      } catch (e) {
        out.push([name, false, `${(buf.byteLength / 1024).toFixed(0)} KB descargados, gzip FALLA: ${(e && e.message) || e}`]);
      }
    } catch (e) {
      out.push([name, false, `no se pudo descargar: ${(e && e.message) || e}`]);
    }
  }
  return out;
}
...
async function gunzip(buffer) {
  if (typeof DecompressionStream !== "function") throw new Error("sin DecompressionStream");
...
export async function fullReport(app, opts = {}) {
  ...
  for (const [name, ok, detail] of assets) row("archivo de avatar: " + name, ok, detail);
  const badAssets = assets.filter((a) => !a[1]);
  if (!badAssets.length) row("archivos de avatar", true, `${assets.length}/${assets.length} cargados`);
