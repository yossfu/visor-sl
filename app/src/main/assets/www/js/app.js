...
import { syncRevision } from "./cache.js";
...
export class App {
  constructor(canvas) {
    ...
    this.ui.showLogin();
    // The copies of grid data kept on the device are addressed by revision: if
    // this build expects different stored data than the last one, the old store
    // is thrown away here, before anything reads it. Without this the viewer can
    // spend a whole test session showing results produced by an old file.
    Promise.resolve()
      .then(() => syncRevision())
      .then((r) => {
        if (r.changed) {
          this.ui.log(`Caché: revisión ${r.rev} (antes ${r.previous}) — ${r.deleted == null ? "se descarta lo guardado" : `borrados ${r.deleted} archivos`} para que nada viejo se cuele en esta prueba.`);
        }
      })
      .catch(() => {});
  }
