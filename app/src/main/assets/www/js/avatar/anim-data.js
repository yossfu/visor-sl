...
//
// The bundle in data/avatar/anims.bin is every shipped animation in one file:
//
...
async function loadBundle() {
  if (!bundlePromise) {
    bundlePromise = (async () => {
      ...
      const bytes = await loadBytes("anims.bin");
      if (String.fromCharCode(bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7]) !== MAGIC) {
        throw new Error("data/avatar/anims.bin no es un paquete de animaciones");
      }
      const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
