...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // One-time per installed build: throw away the WebView's own HTTP cache.
        // App data survives an APK update and so does the cached copy of a file
        // (or of a "404") from the previous build; without this, a page can keep
        // loading an older version of its own assets and every fix made to them
        // looks like it did nothing on the phone while working in the designer.
        val previousBuild = prefs.getInt("visor.httpCacheBuild", -1)
        if (previousBuild != BuildConfig.VERSION_CODE) {
            webView.clearCache(true)
            prefs.edit().putInt("visor.httpCacheBuild", BuildConfig.VERSION_CODE).apply()
            Log.i(TAG, "caché HTTP del WebView vaciada (build ${BuildConfig.VERSION_CODE}, antes $previousBuild)")
        }
        ...
        webView.webViewClient = object : WebViewClient() {
            override fun shouldInterceptRequest(
                ...
                return serveAsset(url.encodedPath ?: "/", request.requestHeaders)
            }
     ...
     */
    private fun serveAsset(encodedPath: String, requestHeaders: Map<String, String>): WebResourceResponse {
        ...
        val etag = etagFor(rel)
        for (candidate in candidates) {
            try {
                val stream = assets.open(candidate, AssetManager.ACCESS_STREAMING)
                // Conditional request: the etag carries the app build, so a new
                // APK always re-sends the body, while a page reload inside the
                // same build costs nothing. This is what keeps a file that has
                // changed from being served from a copy made before the change.
                if (httpHeader(requestHeaders, "If-None-Match") == etag) {
                    try { stream.close() } catch (_: IOException) {}
                    return WebResourceResponse(mimeOf(rel), null, 304, "Not Modified", assetHeaders(etag), null)
                }
                return WebResourceResponse(mimeOf(rel), null, 200, "OK", assetHeaders(etag), stream)
            } catch (_: IOException) {
        ...
        return WebResourceResponse("text/plain", "utf-8", 404, "Not Found", noStoreHeaders(), ByteArrayInputStream(body))
    }
    ...
    private fun httpHeader(headers: Map<String, String>, name: String): String? =
        headers.entries.firstOrNull { it.key.equals(name, ignoreCase = true) }?.value?.trim()

    /**
     * The etag of an asset is the build number plus the path: strong enough to
     * tell one build from the next (which is all that matters for files that
     * only change when the APK does) and stable inside a build.
     */
    private fun etagFor(rel: String): String = "\"b" + BuildConfig.VERSION_CODE + "-" + rel.hashCode() + "\""

    private fun assetHeaders(etag: String): Map<String, String> = mapOf(
        "Cache-Control" to "max-age=0, must-revalidate",
        "ETag" to etag,
        "Access-Control-Allow-Origin" to "*",
    )

    /**
     * A "not found" must never be stored. A cached 404 outlives the fix that
     * added the missing file (the app data survives every APK update), which
     * turns one bad build into a permanent "the file is not there" that no
     * amount of rebuilding can shake off.
     */
    private fun noStoreHeaders(): Map<String, String> = mapOf(
